...
export default function MobileUpcomingEvents({ events }: MobileUpcomingEventsProps) {
  if (events.length === 0) {
    return (
        ...
        {events.slice(0, 4).map((event, eventIndex) => (
          ...
          >
            <Link
              ...
              className="block rounded-lg border transition-all hover:shadow-md hover:border-primary/30 bg-card h-full overflow-hidden"
            >
              {/* Event Image/Header */}
              <div 
