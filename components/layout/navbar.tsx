...
interface SearchResult {
  id: string;
  title: string;
  type: 'event' | 'community' | 'tnp' | 'study';
  handle?: string;
  date?: string;
  image?: string;
  url: string;
      ...
      try {
        ...
        if (response.ok) {
          ...
          const combinedResults = [
            ...(results.events || []).slice(0, 2).map((item: any) => ({
              ...item,
              url: `/events/${item.id}`
            })),
            ...(results.communities || []).slice(0, 2).map((item: any) => ({
              ...item,
              url: `/communities/${item.handle}`
            })),
            ...(results.tnp || []).slice(0, 2).map((item: any) => ({
              ...item,
              url: `/tnp/${item.id}`
            })),
            ...(results.study || []).slice(0, 2).map((item: any) => ({
              ...item,
              url: `/study/${item.id}`
            }))
          ];
        ...
        <div className="flex w-full items-center justify-between md:justify-start">
          ...
          <div className="md:hidden">
            <Link href="/home" className="flex items-center">
              <Image src="/icons/icon-128x128.png" alt="CTC Logo" width={28} height={28} className="rounded-md" />
            </Link>
          </div>
          ...
          <div className="hidden md:flex items-center gap-2 w-[68px] xl:w-[275px] justify-center xl:justify-start px-4">
            <Link href="/home" className="flex items-center gap-2">
              <Image src="/icons/icon-128x128.png" alt="CTC Logo" width={32} height={32} className="rounded-lg" />
              <span className="hidden font-bold xl:inline-block">CTC</span>
            </Link>
          </div>
          ...
          <div className="flex items-center justify-center md:flex-1">
            <div className="flex-1 flex justify-center">
              <div className="relative w-full max-w-md" ref={searchInputRef}>
                ...
                <Input
                  placeholder="Search events, communities, jobs, study..."
                  className="pl-8"
                  value={searchQuery}
                  ...
                  <div className="absolute top-full left-0 right-0 mt-1 z-50 bg-background border border-border rounded-md shadow-lg max-h-96 overflow-hidden">
                    {isSearching ? (
                      ...
                      <div className="py-2">
                        ...
                        {/* TNP Posts */}
                        {searchResults.filter(result => result.type === 'tnp').length > 0 && (
                          <div>
                            <div className="px-3 py-2 text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                              Jobs & Internships
                            </div>
                            {searchResults
                              .filter(result => result.type === 'tnp')
                              .map(result => (
                                <div
                                  key={result.id}
                                  onClick={() => handleSelectSearchResult(result.url)}
                        ...
                        {/* Study Materials */}
                        {searchResults.filter(result => result.type === 'study').length > 0 && (
                          <div>
                            <div className="px-3 py-2 text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                              Study Materials
                            </div>
                            {searchResults
                              .filter(result => result.type === 'study')
                              .map(result => (
                                <div
                                  key={result.id}
                                  onClick={() => handleSelectSearchResult(result.url)}
